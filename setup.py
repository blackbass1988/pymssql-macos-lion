...
if sys.platform == 'win32':
    ...
    # default directories for FreeTDS headers
    include_dirs = filter(os.path.exists, [
        # fink
        '/sw/include',
        # macports
        '/opt/local/include',
    ...
    # default directories for FreeTDS libraries
    library_dirs = filter(os.path.exists, [
        # fink
        '/sw/lib',
        # macports
        '/opt/local/lib',
...
class build_ext(_build_ext):
    ...
    def run(self):
        if sys.platform == 'darwin' and (not include_dirs or not library_dirs):
            print >> sys.stdout, \
                "WARNING: Could not find default directories for FreeTDS " + \
                "headers and/or libraries.  If setup fails, try:\n" + \
                "python setup.py clean build_ext " + \
                "--include-dirs=/path/to/freetds/headers " + \
                "--library-dirs=/path/to/freetds/libraries " + \
                "build install"
